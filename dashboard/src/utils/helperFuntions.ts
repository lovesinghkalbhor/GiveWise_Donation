...
const validateSearchText = (text: string): boolean => {
  ...
  console.log(text.trim(), "here is the text after trim");
  if (text.trim() === "") {
    return false;
  }

  // Check if the string is either a valid number or contains only valid letters (A-Z or a-z)
  const isValid = /^[A-Za-z0-9\s-]+$/.test(text); // modify such that it can also contain white spaces
  return isValid;
};
...
const handleConvertToPDF = (htmlContent: any) => {
  ...
  logoImage.src = "https://anubhutivisionss.com/images/AnubhutiLogo.png";
