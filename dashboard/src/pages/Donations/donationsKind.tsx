      ...
      if (apiData.data) {
        ...
        const formattedDonations = apiData.data.donations.map(
          (donation: any) => ({
            ...
            itemNames: donation.items.reduce(
              (accumulator: string, currentValue: any) => {
                return (
                  accumulator +
                  currentValue?.name +
                  " = Rs " +
                  currentValue?.approxAmount +
                  ", "
                );
              },
              ""
            ...
            // approxAmmount: donation.items.totalItemAmount,
            approxAmmount: donation.items.reduce(
              (accumulator: number, currentValue: any) => {
                return accumulator + currentValue?.approxAmount;
              },
              0
            ),
          })
              ...
              {filteredData.length > 0 ? (
                filteredData.map((donation) => (
                  <tr key={donation.receiptNo}>
                    ...
                    <td>{donation.updatedByPersonName}</td>
