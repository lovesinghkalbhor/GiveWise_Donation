...
const EditDonationFormKinds: React.FC = () => {
  const { id } = useParams();
  const { donationData } = useKindDonationData(String(id));
  ...
  const filteredItems = (items: any[]) => {
    ...
    const itemsArray = items.filter((item) => {
      ...
      const isApproxAmountEmpty = String(item.approxAmount).trim();
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      console.log(apiData, "here is the api data");
      if (apiData.data) {
...
export default EditDonationFormKinds;
